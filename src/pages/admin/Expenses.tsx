...
import {
  Search, Trash2, Plus, Upload, Undo, Calendar as CalendarIcon
} from 'lucide-react';
              ...
              <Table>
                <TableHeader className="sticky top-0 z-10 bg-background">
                  <TableRow>
                    ...
                    <TableHead onClick={() => requestSort('description')} className="cursor-pointer min-w-[300px] md:w-[400px]">Description</TableHead>
                    <TableHead onClick={() => requestSort('amount')} className="cursor-pointer text-right">Amount</TableHead>
                  ...
                  {addingNew && (
                    <TableRow className="bg-muted/50">
                      ...
                      <TableCell className="p-1">
                        <Input
                          type="date"
                          value={newExpense.date || format(new Date(), 'yyyy-MM-dd')}
                      ...
                      <TableCell className="p-1">
                        <Select
                          ...
                          onChange={(opt: any) => setNewExpense(prev => ({ ...prev, category: opt.value }))}
                          styles={getSelectStyles(isDarkMode)}
                        />
                      </TableCell>
                      <TableCell className="p-1 ">
                      ...
                      <TableCell className="p-1">
                        <div className="flex gap-1">
                          <Button size="sm" onClick={handleAddNew} className="h-8 px-2">Save</Button>
                          <Button size="sm" variant="outline" onClick={() => {
                            setAddingNew(false);
                            setNewExpense({ date: format(new Date(), 'yyyy-MM-dd'), amount: 0, category: '', description: '' });
                          }} className="h-8 px-2">Cancel</Button>
                        </div>
                      </TableCell>
                    </TableRow>
                  )}
                      ...
                      <TableCell>
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => {
                            if (checked) {
                              setSelectedRows(prev => [...prev, expense.id]);
                            } else {
                              setSelectedRows(prev => prev.filter(id => id !== expense.id));
                            }
                          }}
                        />
                      </TableCell>
